...
- **布局**：房间排在网格上，主干从左到右（**只走正交步**），
  偶尔上下错开让地图看起来分散；额外房间作为分支挂在主干上，是可选奖励。
...
`test_run` §16 校验"贴出来的瓦片和模板一致"（抓坐标偏移一格），
§17 跨 24 个种子校验"没有敌人/宝箱生在墙里"。
...
**改这里之后必须跑 `test_run` §16**：它逐格比对"贴出来的瓦片 vs 模板"，
...
参数靠**反射**从 Player 与当前武器的数值型 `@export` 自动发现，加新参数无需改面板。
按键避开移动与射击，可边跑边打边调。

调好后把数值手工写回 `player.gd` 默认值或 `WEAPONS` 表，再重跑生成器。
