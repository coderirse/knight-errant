...
> `tools/test_*.gd` 的自动化测试背书（两套共 182 项断言，全绿）。
...
| 阶段 | 目标 | 验收 |
|---|---|---|
| **M0 骨架** ✅ | 双摇杆、武器、敌人、程序生成、HUD、存档、测试 | `test_gameplay` + `test_run` 全绿（交付时 96 项；当前 182 项） |
...
两套共 **182 项**，判定标准是 stdout 最后一行 `ALL NN CHECKS PASSED`。
...
#       场景切换入口（P0 回归）、永久升级生效、调参台、模板空间校验、灯光、布局形状、小地图、三层完整循环（112 项）
